...
void glcd_image_disp(unsigned char* img_src)
{
	unsigned char page, column, data_index;
	//unsigned char i,j;
...
void main()
{
	...
	while(1)
	{
		...
		if((left_right_avg <200) || (up_down_avg<200))
		{
			delay_cnt = 670000/5;
		}
		else if((left_right_avg >3900) || (up_down_avg>3900))
		{
			delay_cnt = 670000*5;
		}
		else
		...
		for(page=0; page<8; page++)
		{
			...
			{
				glcd_setcolumn(column);
				glcd_data(one[data_index]);
				data_index++;
			}
		}
		SysCtlDelay(delay_cnt);
